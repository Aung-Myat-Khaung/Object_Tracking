        ...
        for d_idx, detection in enumerate(detections):
            if d_idx in used_detection:
                ...
                if (dist<MAX_DISTANCE):
                    state = d_idx
                    diff_d = dist
                    found.append([state,diff_d])
        if len(found)>0:
            min_d = found[0][1]
            index = found[0][0]
            for state,diff_d in found:
                if (diff_d<min_d):
                    index = state
            tracker.update_detection(frame,detections[index])
